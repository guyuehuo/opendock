...
def worker(cpu_core_index, args, ligand, receptor, sf,init_lig_cnfrs, init_recp_cnfrs,xyz_center, box_sizes, sampler,
               num_samples, results_cnfrs, results_scores, process_id,ntasks,len_cnfrs,cnstr):
...
def main():
    ...
    #print("1 Ligand XYZ COM", xyz_center)
    box_sizes = float(configs['size_x']), \
                float(configs['size_y']), float(configs['size_z'])

    # define a flexible ligand object
    ligand = LigandConformation(configs['ligand'])
    xyz_center1 = ligand._get_geo_center().detach().numpy()[0]
    xyz_center = float(xyz_center1[0]), \
        float(xyz_center1[1]), float(xyz_center1[2])
    #print("2 Ligand XYZ COM", xyz_center)
   ...
   #set constrain
    ...
    # indices_r = asl.select_atom(atomnames=['CA', ], chains=['B'], residx=['215'],resnames=['TRP'])
    # indices_r = asl.select_atom(atomnames=['CA', ], chains=['A'], residx=['157'],resnames=['ALA'] )
    ...
    file_path = os.path.join(configs['out'], 'constrain_8mc_output_clusters_sort=vina.pdbqt')
    ...
    while num_processes>=available_cpu_cores*2:
      ...
      for i in range(available_cpu_cores*2):
        ...
        results_cnfrs, results_scores, i ,ntasks,len_cnfrs,cnstr))
        processes.append(p)
        p.start()
      for p in processes:
        p.join()
    ...
    if num_processes%available_cpu_cores!=0:
        ...
        for i in range(num_processes):
            ...
            p = multiprocessing.Process(target=worker, args=(
                ...
                sampler, num_samples,results_cnfrs, results_scores, i, ntasks, len_cnfrs,cnstr))
            processes.append(p)
            p.start()
        for p in processes:
            p.join()
    ...
    write_ligand_traj(_cnfrs_list_vina, ligand,
                      os.path.join(configs['out'], 'constrain_8mc_output_clusters_sort=vina.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd, ligand,
                      os.path.join(configs['out'], '8mc_output_clusters_sort=deeprmsd.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd_vina, ligand,
                      os.path.join(configs['out'], '8mc_output_clusters_sort=deeprmsd_vina-0.8.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd_vina, ligand,
                      os.path.join(configs['out'], '8mc_output_clusters_sort=deeprmsd_vina-0.5.pdbqt'),
                      information={"DeepRmsd-vinaScore": _scores_deeprmsd_vina},
                      )
    time_end = time.time()
