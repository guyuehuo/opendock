...
class GeneticAlgorithmSampler(BaseSampler):
    ...
    def __init__(self,
        ...
        #self.n_bit = kwargs.pop("n_bit", [25, ] * self.n_var)
        self.n_bit = kwargs.pop("n_bit", [12, ] * self.n_var)
        print("self.n_bit",self.n_bit)
        ...
        if self.ligand.cnfrs_ is not None:
            ...
            #self.bound += [[-1.0, 1.0] for x in range(3)]
            #self.bound += [[-1. * np.pi, np.pi] for x in range(self.n_var - 3)]
            self.bound += [[-1. * 3.0, 3.0] for x in range(self.n_var - 3)]
        else:
            self.bound = kwargs.pop("bound", [[-1. * np.pi, np.pi] for x in self.n_var])
        # print(self.bound)
        self.lb = [x[0] for x in self.bound]
        self.ub = [x[1] for x in self.bound]
        ...
        print("self.chrom_size",self.chrom_size)
            ...
            def make_chrom():
                ...
                for i in range(self.chrom_size):
                    ...
                    if _p > 0.8 and i % 5 == 0:
                        ...
                        _chrom[i] = int((int(_chrom[i]) + 1 <= 1) * 1)
    ...
    @staticmethod
    def decode1(chrom, low=0, high=1,precision=3):
        ...
        for i,v in enumerate(chrom):
            ...
            if v==1.0:
               ...
               chrom[i] = '0'
        #print("chrom",chrom)
        binary_string = "".join(chrom)
        #binary_string=chrom
        scaled_integer = int(binary_string, 2)

        # Map the integer back to the original floating point range
        floating_number = float(scaled_integer / 10 ** precision)
        if flag==1.0:
            floating_number=floating_number*(-1.0)
        #print("floating_number",floating_number)
    ...
    def binary_code(val, n_bit, low=-1, high=1):
        ...
        if high != low:
            try:
                ind_bin = int(round((val - low) / (high - low) * (ind_bin_max - 1)))
            except:
    ...
    @staticmethod
    def binary_code1(val, n_bit, low=-1, high=1, precision=3):
        """
        binary encoding a float value
        ...
        binary_val : array
            binary representation of the passed float value.
        """
        # Step 1: Map the floating number to an integer
        flag='0'
        #print("val",val)
        scaled_integer = int(val * 10 ** precision)

        # Step 2: Convert the integer to binary
        #print("bin(scaled_integer)",bin(scaled_integer))
        if val>0:
            binary_representation = bin(scaled_integer)[2:]
        else:
            binary_representation = bin(scaled_integer)[3:]
            flag='1'
        #print()

        # Add leading zeros to achieve desired precision
        while len(binary_representation) < 24:
            binary_representation = '0' + binary_representation
        binary_representation=flag+binary_representation
        #print("binary_representation",binary_representation)
        binary_val = np.zeros(25)
        for i in range(25):
            # binary_val[i]=int(binary_representation[i])
            binary_val[i] = binary_representation[i]
        # binary_val[24]=int(flag)
        #binary_val[24] = flag
        #print("binary_val",binary_val)
        return binary_val
        #return  flag+binary_representation
