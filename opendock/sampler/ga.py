...
class GeneticAlgorithmSampler(BaseSampler):
    ...
    def __init__(self,
        ...
        if self.ligand.cnfrs_ is not None:
            ...
            # rotation (3) + torsions (k) are searched over a full angular
            # range so distant de-novo conformers can be recovered.
            self.bound += [[-1 * self.bound_value, self.bound_value]
                           for x in range(self.n_var - 3)]
        else:
