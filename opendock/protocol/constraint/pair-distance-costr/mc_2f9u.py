...
def main():
    ...
    indices_r = asl.select_atom(atomnames=['CA', ], chains=['A'], residx=['42'], resnames=['THR'])
    print("indices_r",indices_r)
    #print(indices_r, receptor.dataframe_ha_.head())
    print(indices_r, receptor.dataframe_ha_.iloc[580])

    asl = AtomSelection(molecule=ligand)
    # indices_l = asl.select_atom(atomnames=['C1', ])
    indices_l = asl.select_atom(atomnames=['C43', ])
    ...
    write_ligand_traj(_cnfrs_list_vina, ligand,
                      os.path.join(configs['out'], 'c10-constrain_8mc_output_clusters_sort=vina.pdbqt'),
