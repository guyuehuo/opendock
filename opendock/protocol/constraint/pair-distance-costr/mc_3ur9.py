...
def main():
    ...
    indices_r = asl.select_atom(atomnames=['CA', ], chains=['A'], residx=['160'],resnames=['ALA'] )
    print("indices_r",indices_r)
    #print(indices_r, receptor.dataframe_ha_.head())
    print(indices_r, receptor.dataframe_ha_.iloc[660])

    asl = AtomSelection(molecule=ligand)
    indices_l = asl.select_atom(atomnames=['C24', ])
    print("indices_l",indices_l)
    #print(indices_l, ligand.dataframe_ha_.head())
    print(indices_l, ligand.dataframe_ha_.iloc[16])
