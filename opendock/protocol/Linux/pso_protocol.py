...
def worker(cpu_core_index, ligand, receptor, sf, init_lig_cnfrs, init_recp_cnfrs, xyz_center, box_sizes,
    ...
    while not is_a_success_sampling and torlence < 5:
       ...
       pso= ParticleSwarmOptimizer(ligand, receptor, sf, social_param=0.5,
                                 ...
                                 minimization_ratio=1,
                                 population_size=3,
                                 early_stop_tolerance=22,
                                 )
       #pso._random_move(init_lig_cnfrs, pso.receptor.cnfrs_)
       print(f"[INFO] ParticleSwarmOptimizer Round #{process_id}")
       #times=ligand.number_of_heavy_atoms*20
       times=25
       # times = 5
       _,_,is_a_success_sampling=pso.sampling(times)
       torlence += 1
       #is_a_success_sampling=True

    sorted_pairs = sorted(zip(pso.ligand_scores_history_, pso.ligand_cnfrs_history_), key=lambda pair: pair[0])
    res = sorted_pairs[:20]
    last_20_elements = [
    ...
    # results_cnfrs += pso.ligand_cnfrs_history_
    # results_scores += pso.ligand_scores_history_
...
def main():
    ...
    print("collected_scores", collected_scores[:20])
    #exit()
    ...
    write_ligand_traj(_cnfrs_list_vina, ligand,
                      os.path.join(configs['out'], 'test_pso_output_clusters_sort=vina.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd, ligand,
                      os.path.join(configs['out'], 'pso_output_clusters_sort=deeprmsd.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd_vina, ligand,
                      os.path.join(configs['out'], 'pso_output_clusters_sort=deeprmsd_vina.pdbqt'),
    ...
    write_ligand_traj(_cnfrs_list_deeprmsd_vina, ligand,
                      os.path.join(configs['out'], 'pso_output_clusters_sort=deeprmsd_vina-0.5.pdbqt'),
                      information={"DeepRmsd-vinaScore": _scores_deeprmsd_vina},
                      )
