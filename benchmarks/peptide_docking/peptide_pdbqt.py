#!/usr/bin/env python
"""Core: turn a (cyclic) peptide into an OpenDock-ready PDBQT whose backbone /
macrocyclic ring is held rigid and only the side-chain chi dihedrals are
rotatable.

Residue / fragment / backbone / ring definitions come from the *porality*
package (~/apps/porality):
...
torsion tree (``opendock/core/ligand.py``, ``conformation.py``): only
``BRANCH`` bonds rotate.  This module emits a PDBQT whose ``ROOT`` is the whole
rigid backbone (for cyclic peptides the complete macrocyclic ring) and whose
``BRANCH`` records encode exactly the flexible side-chain bonds.

Freeze rule
-----------
A heavy-atom bond is flexible iff ALL of:

* it is a single bond,
* it is *not* part of any ring  -> any dihedral whose central bond lies on a
  macrocyclic backbone ring is never flexible,
* removing it does not disconnect the porality backbone atom set
  (``N/CA/C/O`` per residue plus ring-closure linkages) - excludes the
  phi/psi/omega backbone dihedrals of linear peptides,
* the side that carries no backbone atom holds >= 2 heavy atoms (excludes
  terminal methyl / hydroxyl / thiol rotations that leave the heavy geometry
  unchanged).

The oriented flexible bonds always form a forest rooted on the backbone
component.  Every connected component of the graph after removing the flexible
bonds is one rigid group; the PDBQT is written depth-first so OpenDock's frame
slicing (atoms between consecutive ``BRANCH`` records) sees exactly one rigid
group per frame.
"""
...
try:
    ...
    raise ImportError("peptide_pdbqt requires rdkit (conda-forge rdkit)") from e
...
try:
    ...
    raise ImportError(
        "peptide_pdbqt requires the 'porality' package (~/apps/porality). "
        "Install it with: pip install -e ~/apps/porality") from e

AD4_HYDROGEN_TYPES = ("H", "HD")


def log(msg):
    print(f"[peptide_pdbqt] {msg}", flush=True)
...
def find_mgltools():
    """Locate MGLTools (pythonsh + prepare_ligand4.py), mirroring the PDBbind
    benchmark harness (benchmarks/pdbbind_casf2016/01_prepare_inputs.py)."""
...
def generate_typed_pdbqt(mol, out_pdbqt, tools=None, workdir=None):
    ...
    OpenBabel first (as in benchmarks/pdbbind_casf2016/01_prepare_inputs.py).
    The typed heavy atoms keep the coordinates/elements of `mol`; their order
    may change (we map back by coordinates later).
    """
...
def _element_of_ad4(ad4):
    ...
    if sym == "A":
        return "C"
    if len(ad4) > 1 and ad4[1].islower():
        return ad4[:2].capitalize() if ad4[:2] in ("cl", "br") else ad4[0]
...
def write_frozen_pdbqt(mol, flexible_pairs, heavy_by_mol, h_records,
    ...
    def collect(cid):
        ...
        for mi in sorted(comps[cid]):
            toks.append(("atom", mi))
            for _ in _h_lines_for(h_records, mi):
                toks.append(("h", mi))
    ...
    heavy_serial = {}
    for (kind, mi), ser in zip(flat, serials):
        if kind == "atom":
            heavy_serial[mi] = ser

    cursor = {"i": 0}

    def token_line(kind, mi):
        ser = serials[cursor["i"]]
        cursor["i"] += 1
        rec = heavy_by_mol[mi] if kind == "atom" else _h_lines_for(h_records, mi)[0]
...
def prepare_peptide_pdbqt(input_path=None, smiles=None,
    ...
    typing -> topology rewrite. Returns (model, meta_dict)."""
